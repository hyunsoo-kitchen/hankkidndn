<template>
    <div class="container">
        ...
        <div class="main_list">
            ...
            <!-- 댓글 불러오기 시작 -->
              <div v-if="$store.state.commentData" v-for="(item, index) in $store.state.commenData" :key="index" class="comment">
              ...
              <!-- 대댓글 불러오기 시작 -->
              <div v-for="(item2, index2) in $store.state.cocomentData" :key="index2">
                <div v-if="item2.cocomment == item.id" class="comment">
                    <div v-if="item2.id !== cocommentId">
                        ...
                        <p v-if="!item2.deleted_at" class="comment-conetne">{{ item2.content }}</p>
                        <p v-else>삭제된 댓글 입니다.</p>
...
function likeToggle(commentData) {
    console.log(commenData)
    if(commenData.like_chk == 1) {
        commentData.like_chk = 0;
        commenData.likes_num--;
    } else {
        commentData.like_chk = 1l
